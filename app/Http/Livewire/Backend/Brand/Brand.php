...
class Brand extends Component
{
    ...
    {
        return view('livewire.backend.brand.brand');
    }

    public function restore($id)
    {
        ModelsBrand::withTrashed()->find($id)->restore();
    ...
    public function deleteSelected()
    {
        ...
        if ($this->force === false) {
            ModelsBrand::whereIn('id', $this->selectedItems)->delete();
        } else {
            ModelsBrand::onlyTrashed()->whereIn('id', $this->selectedItems)->forceDelete();
        }
        $this->clear();
    ...
    {
        if ($this->force === false) {
            ModelsBrand::where('id', $this->selectedItem)->delete();
        } else {
            ModelsBrand::onlyTrashed()->find($this->selectedItem)->forceDelete();
        }
        $this->dispatchBrowserEvent('success-izi', ['ntitle' => 'Success', 'nmessage' => "Brand has been deleted"]);
        $this->clear();
        $this->emit('refresh');
        return FALSE;
    }
